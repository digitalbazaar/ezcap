...
export class ZcapClient {
   ...
   * @param {object} options.invocationKeypair - An public/private keypair
   *   can be used to generate an invocation signer -- a function with a
   *   `.sign()` function and key `id` parameter that will be used for
   *   signing the request.
   * @param {object} options.delegationKeypair - An public/private keypair
   *   can be used to digitally sign a delegation.
   *
   * @returns {ZcapClient}.
   */
  constructor({
    baseUrl, defaultHeaders = {}, agent, invocationKeypair, delegationKeypair
  } = {}) {
    const invocationSigner = invocationKeypair.signer();
    invocationSigner.id = invocationKeypair.id;

    this.baseUrl = baseUrl;
    this.agent = agent;
    this.defaultHeaders = {...DEFAULT_HEADERS, ...defaultHeaders};
    this.invocationSigner = invocationSigner;
    this.delegationKeypair = delegationKeypair;
  }
    ...
    const signedDelegatedCapability = await jsigs.sign(
      delegatedCapability, {
        suite: new Ed25519Signature2018({
          key: this.delegationKeypair,
          // TODO: expires doesn't appear in signed document
          proof: {
            '@context': SECURITY_CONTEXT_V2_URL,
            expires
          }
