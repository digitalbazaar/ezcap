...
export class ZcapClient {
  ...
  async request({
    ...
    if(capability === 'string') {
      ...
      const prefixIdx = capability.startsWith(ZCAP_ROOT_PREFIX);
      if(prefixIdx !== 0) {
        throw new Error(
          'When "capability" is a string, it must be a root authorization ' +
          'capability.');
      }
      // extract required properties from root zcap ID
      const invocationTarget = decodeURIComponent(capability.substr(prefixIdx));
    ...
    } else if(capability !== undefined && !_isZcap({capability})) {
      throw new Error(
