...
/**
  * A Class that is capable of generating a Linked Data Signature.
  *
...
export class ZcapClient {
   ...
   *   used for signing requests. `invocationSigner` and `delegationSigner`, or
   *   `didDocument` and `keyPairs` must be provided.
   * @param {object} [options.suite] - The default signature suite to use when
   *   digitally signing delegations. Default: Ed25519Signature2018.
   *
   * @returns {ZcapClient} - The new ZcapClient instance.
   */
  constructor({
    baseUrl, defaultHeaders = {}, agent, didDocument, keyPairs,
    invocationSigner, delegationSigner, suite = Ed25519Signature2018
  } = {}) {
   ...
   * @param {Function<LinkedDataSignature>} [options.suite] - Optional signature
   *   suite to use when digitally signing the delegation. Default: The
   *   signature suite that was used when initializing the ZcapClient.
   *
  ...
  async delegate({
    ...
    const SignatureSuite = suite || this.suite;
