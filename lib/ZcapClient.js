...
export class ZcapClient {
   ...
   *  `url` or `capability` must be specified; `url` is only permitted if
   *   delegating a root zcap.
   * @param {string|object} [options.capability] - The parent capability to
   *   delegate; must be an object if it is a delegated zcap, can be a string
   *   if it is a root zcap; either `url` or `capability` must be specified;
   *   `url` is only permitted if delegating a root zcap.
  ...
  async delegate({
    ...
    if(capability) {
      ...
      // generate root zcap ID from `url`
      parentCapability = await generateZcapUri({url});
    }

    if(invocationTarget === undefined) {
      if(typeof capability === 'string' && !url) {
        throw new Error(
          '"invocationTarget" must be specified when "capability" is ' +
          'a string and "url" is not given.');
      }
      // use `capability` target first, if given, otherwise use `url`
      invocationTarget = (capability && capability.invocationTarget) || url;
    ...
    const signedDelegatedCapability = await jsigs.sign(
      delegatedCapability, {
        ...
        purpose: new CapabilityDelegation({
          ...
          parentCapability: capability || url
        })
      });
  ...
  async request({
    ...
    if(capability) {
      invocationTarget = (capability.invocationTarget &&
        (capability.invocationTarget.id || capability.invocationTarget));
